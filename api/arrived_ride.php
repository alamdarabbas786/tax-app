...
try {
    ...
    $stmt = $pdo->prepare('UPDATE rides
        ...
        WHERE id = ? AND driver_id = ? AND status IN ("accepted","searching")');
    $stmt->execute([$rideId, $driverId]);
    if ($stmt->rowCount() !== 1) {
        json_response(409, ['status' => 'error', 'message' => 'Ride cannot be marked arrived']);
        exit;
    }
    ...
    $published = emit_ride_event('driver_arrived', $rideId, ['driver_id' => $driverId, 'customer_id' => $customerId]);
    notification_insert(
        $pdo,
        $rideId,
    ...
    if (!$published && $customerId > 0) {
        $fallback = fcm_send_to_entity(
            ...
            'customer',
            $customerId,
            ['title' => 'Driver Arrived', 'body' => 'Your driver has arrived at pickup point'],
            ['ride_id' => (string)$rideId, 'status' => 'arrived'],
            60,
            ['role' => 'customer', 'recipient_id' => $customerId, 'event_type' => 'driver_arrived']
        );
    }

    json_response(200, ['status' => 'ok', 'message' => 'Driver marked arrived', 'fallback_fcm' => $fallback]);
} catch (Throwable $e) {
